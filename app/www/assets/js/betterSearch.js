...
function betterSearchFn(tipo, target, alvo, tipoDados){
    ...
    if(tipo=="listaBancos"){
       $(".header-better-search span").html("LISTA DE BANCOS");
       listaBancos();
    }
...
async function listaBancos(){
    ...
    //betterSearch = await fetch('https://tepping-api-bancos.now.sh/').then(res => res.json());
    betterSearch = [
  {
    "Code": "001",
  ...
  },
  {
    ...
    if(betterSearch.length>0){
          //console.log("API BETTER SEARCH LISTA DE BANCOS");
          //console.log(betterSearch);
          
          for(var i = 0;i<betterSearch.length;i++){
            $("#betterSearch nav ul").append("<li onclick='$(\"#listaBancos\").val($(this).text()); $(\"#labelListaBancos\").html($(this).text()); fecharBetterSearch();'>"+betterSearch[i].Code+" - "+betterSearch[i].Name+"</li>");
          }
...
function filtroBetterSearch(){
      ...
      ul = document.getElementById("dadosResultadosPesquisa");
